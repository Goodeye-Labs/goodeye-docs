...
Anonymous callers (no auth) who reach public REST endpoints that consume credits have their own small monthly grant, tracked per network address. This covers use cases like running a verifier against a published template without signing in. Total anonymous usage is also bounded by a platform-wide daily limit; once it is reached, anonymous credit-consuming calls return `402 anonymous_daily_cap` until the limit resets at UTC midnight. Signing in gives you your own credits.
